...
class SparseFeatureMapper2(Block):
    ...
    def build(self, hp, inputs=None):
        ...
        # than 10000
        embedding_dim = self.embedding_dim or hp.Choice('embedding_dim', [8, 16], default=8)
        output_node = tf.keras.layers.Embedding(1000, embedding_dim)(input_node[0])
        print(output_node, '1')

        output_node = tf.keras.layers.Dense(embedding_dim)(output_node)
        print(output_node, '2')
...
class BPREmbed(Block):
    ...
    def build(self, hp, inputs=None):
        ...
        # than 10000
        embedding_dim = self.embedding_dim or hp.Choice('embedding_dim', [8, 16], default=8)
        output_node = tf.keras.layers.Embedding(self.hash_size, embedding_dim)(input_node[0])
