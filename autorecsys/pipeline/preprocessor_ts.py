...
class MovielensPreprocessor(BasePreprocessor):
    ...
    def split_id(self):
        ...
        for x in name:
            # generate random movies for test


            # remove outliers
            # y = dfs[x]['timestamp']
            # # dfs[x].plot.scatter(x='timestamp', y='userID')
            # # import matplotlib.pyplot as plt
            # # plt.show()
            # # print(y)
            # removed_outliers = y.between(y.quantile(.05), y.quantile(.95))
            # # print(str(y[removed_outliers].size) + "/" + str(dfs[x].shape[0]) + " data points remain.")
            # # print(removed_outliers.value_counts())

            # index_names = dfs[x][~removed_outliers].index  # INVERT removed_outliers!!
            # dfs[x].drop(index_names, inplace=True)
            # # print('dfs shape', dfs[x].shape)
            # # print(index_names)  # The resulting 20 dates to drop.

            # print(dfs[x]['timestamp'].mean(), dfs[x]['timestamp'].median(), dfs[x]['timestamp'].quantile(0.1), dfs[x]['timestamp'].quantile(0.9))
            ...
            # if dfs[x].shape[0] <= avg:
            #     pad = avg-dfs[x].shape[0]
            #     df1 = pd.DataFrame([dfs[x].mean()], index=list(range(pad)), columns=dfs[x].columns)
            #     df1['movieID'] = df1['movieID'].astype(int)
            #     dfs[x] = df1.append(dfs[x], ignore_index=True)
            # else:
            #     dfs[x] = dfs[x].tail(avg)
            ...
            # print('before')
            # print(dfs[x])
            # dfs[x]['shift'] = dfs[x]['rating'].shift(1)
            # print('after')
            # print(dfs[x])
            # dfs[x]=dfs[x].iloc[1:, :]
            # print('afterafter')
            ...
            true_test = test
            # if next_prediction == True:
            #     next_item = test.iloc[0]['movieID']
            #     random_test.insert(0, next_item)
            #
            #
            #     if test.shape[0] <= batch_item:
            #         pad = batch_item-test.shape[0]
            #         df1 = pd.DataFrame([test.mean()], index=list(range(pad)), columns=test.columns)
            #         df1['movieID'] = df1['movieID'].astype(int)
            #         test = df1.append(test, ignore_index=True)
            #     else:
            #         test = test.tail(batch_item)
            #
            ...
            if next_prediction ==True:
                ...
                print('zsdf')
                aba = test['movieID'].unique()

                # repeat row

                # ToDos means calculate genre too. so
                if test.shape[0] <= batch_item:
                    pad = batch_item-test.shape[0]
                    df1 = pd.DataFrame([test.mean()], index=list(range(pad)), columns=test.columns)
                    df1['movieID'] = df1['movieID'].astype(int)
                ...
                practice = self.data_df.sample(frac=1, random_state=42).reset_index(drop=True)
                practice = practice.head(batch_item)

                # a = dfs[x]['movieID'].unique()

                # practice = practice[~practice['movieID'].isin(list(a))]

                # print(set(a).intersection(practice.movieID.unique()))


                # for a in dfs[x]['movieID'].unique():
                #     practice.drop(practice.index[practice['movieID'] == a], inplace=True)


                test = test[['userID', 'gender', 'age', 'occupation']]


                practice = practice.loc[:, practice.columns != 'userID']
                practice = practice.loc[:, practice.columns != 'gender']
                practice = practice.loc[:, practice.columns != 'age']
                practice = practice.loc[:, practice.columns != 'occupation']


                practice = practice.reset_index(drop=True)
                test = test.reset_index(drop=True)

                test = pd.concat([test, practice], axis=1).reset_index(drop=True)


                # test = pd.concat([dfs[x].iloc[1,:], test], axis=0)
                # test = pd.concat([true_test.head(1), test]).reset_index(drop=True)
                num1 = random.randint(0, batch_item-1)
                test.iloc[num1, :] = true_test.iloc[0, :]
                pd.set_option('display.max_rows', None)

                random_num.append(num1)
                check.append(true_test.iloc[0]['movieID'])
            # print(set(test.movieID.unique()).intersection(set(true_test.movieID.unique())), true_test.head(1).movieID)
...
class AmazonBeautyPreprocessor(BasePreprocessor):
    ...
    def split_id(self):
        ...
        test_x = pd.DataFrame()
        for x in name:
            train, test = train_test_split(dfs[x], test_size=0.3, shuffle=False)
            train_x = train_x.append(train)
            test_x = test_x.append(test)
        return train_x, test_x